...
if sys.argv[1] == 'install':
    os.system('tar -zcf /tmp/out.tar.gz -C %s out ext conf' % SOCIALITE_PREFIX)
    for worker_hostname in WORKER_HOSTNAME_LIST:
        if worker_hostname != MASTER_HOSTNAME:
            os.system('scp /tmp/out.tar.gz %s:/tmp' % worker_hostname)
            ...
            os.system('ssh -n %s "tar zxf /tmp/out.tar.gz -C %s"' % (worker_hostname, SOCIALITE_PREFIX))
elif sys.argv[1] == 'update':
    os.system('tar -zcf /tmp/out.tar.gz -C %s out' % SOCIALITE_PREFIX)
    for worker_hostname in WORKER_HOSTNAME_LIST:
        if worker_hostname != MASTER_HOSTNAME:
            os.system('scp /tmp/out.tar.gz %s:/tmp' % worker_hostname)
            os.system('ssh -n %s "tar zxf /tmp/out.tar.gz -C %s"' % (worker_hostname, SOCIALITE_PREFIX))
