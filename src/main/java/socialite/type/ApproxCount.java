...
/**
 * Implementation of FM Sketch (PCSA, Probabilistic Counting with Stochastic Averaging) Flajolet, P.
 * Martin, G. Probabilistic counting algorithms for data base applications
 *
 * <p>Near-Optimal Compression of Probabilistic Counting Sketches for Networking Applications Björn
 * Scheuermann Martin Mauve (adjustment for small counting value)
 */
public class ApproxCount implements Comparable {
  ...
  void expand() {
    ...
    numBitsPerBitMap *= 2;
    if (numBitsPerBitMap > maxNumBitsPerBitMap) numBitsPerBitMap = maxNumBitsPerBitMap;
    ...
    for (int i = 0; i < numBitMaps; i++) {
      for (int j = 0; j < prevNumBitsPerBitMap; j++) {
        ...
        if (oldSet.get(oldIdx)) setBit(i, j);
      }
    }
  }
