...
public class Utf8 implements Comparable<Utf8>, CharSequence, Externalizable {
  ...
  public boolean equals(Object o) {
    ...
    Utf8 u = (Utf8) o;
    if (bytes.length != u.bytes.length) return false;

    byte[] b1 = bytes;
    byte[] b2 = u.bytes;
    for (int i = 0; i < bytes.length; i++) if (b1[i] != b2[i]) return false;
  ...
  public int compareTo(Utf8 that) {
    ...
    if (that.bytes.length < len) len = that.bytes.length;
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    bytes = new byte[len];
    for (int i = 0; i < len; i++) bytes[i] = in.readByte();
  }
}
