...
public class DistEngine {
  ...
  void transferClassFileBlobs(LinkedHashMap<String, byte[]> classes) {
    ...
    try {
      ...
      Method get = WorkerCmd.class.getMethod("addClassFiles", BytesWritable.class);
  ...
  static volatile TupleReqListener _tupleListen = null;
  ...
  void runQuery(CodeGenMain codeGen, TupleSend qv) {
    ...
    if (p.first() instanceof Const) {
      ...
      for (int i = 0; i < workerAddrMap.size(); i++) {
        ...
        if (i == workerAddrMap.size() - 1) tupleReqListener.setInvokeFinish(iterId.get(), true);
        success =
            requestRunQuery(cmd, new IntWritable(t.id()), new Text(queryClassName), iterId, args);
        if (!success) break;
  ...
  boolean requestRunQuery(
      WorkerCmd cmd,
      IntWritable tid,
      Text queryClass,
      LongWritable iterId,
      ConstsWritable constArgs) {
    try {
  ...
  void run(Epoch e) {
    ...
    Method run = null;
    try {
      run = WorkerCmd.class.getMethod("run", EpochW.class);
