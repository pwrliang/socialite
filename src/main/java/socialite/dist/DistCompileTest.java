...
public class DistCompileTest {
  ...
  static void triangle2() {
    ...
    DistEngine en = new DistEngine(fakeMap(), null);
    String query =
        "Edge[int s:0..9]((int t)) sortby t."
            + "Edge[s](t) :- id=$MyId(), b=id, e=id, line=$ReadSplitted(\"hdfs:///user/jiwon/data/edges%d.txt\", b, e),"
            + "      (s1, s2)=$Split(line), s=$ToInt(s1), t=$ToInt(s2)."
            + "Node[int s:0..268435456](int i) predefined.\n";

    String mainQuery1 =
        "Triangle[int h:0..15](int cnt).\n"
    ...
    String finalQuery =
        "TotalTriangle[int n:0..0](int cnt).\n"
  ...
  static void pagerank() {
    ...
    String mainQuery =
        String.format(
            "Rank[int n:0..%d]((int i:0..9, double rank)).\n"
  ...
  static void pagerank_() {
    String initQuery =
        "OutEdge[int s:0..268435456]((int t:1)) sortby t.\n"
            ...
            + "EdgeCnt[s]($Inc(1)) :- OutEdge[s](t).";
    String mainQuery =
        "Rank[int n:0..268435456](double rank).\n"
  ...
  static void lcc() {
    ...
    long start = System.currentTimeMillis();
    String initQuery =
        "Edge[int s:0..268435456]((int t:1)) sortby t.\n"
            + "EdgeCnt[int s:0..268435456](int cnt).\n"
  ...
  static void pr() {
    ...
    String query =
        "InEdge[int s:0..100]((int t)) sortby t.\n"
  ...
  static void triangle() {
    ...
    String query =
        "Edge[int s:0..100]((int t)) sortby t.\n"
    ...
    query +=
        "Filter[int n:0.."
            + (size - 1)
            + "]((int s:0..100, ApproxSet set)).\n"
            +
            // "Filter[mid](s, $ApproxSet(t)) :- Edge[s](t), mid = $Range(0,"+size+").\n";
            "";

    query +=
        "Triangle[0]($Inc(1)) :- Edge[x](y), Edge[x](z), id=$MyId(), Filter[id](y, set),"
  ...
  static void gd() {
    ...
    String initQuery =
        "Rating[int u:0.."
            + (M - 1)
            + "]((int v, double r)) sortby v, multiset.\n"
            +
            ...
            + ", double p)) groupby(3).\n"
            +
            // "V[int v:0.." + (N - 1) + "](int w:iter, (int l:0.." + (L - 1) + ", double q))
            // groupby(3).\n" +
            "Err[int u:0.."
            + (M - 1)
            + "]((int v, double e)) sortby v, groupby(2).\n"
            +
            // "ErrT[int v:0.." + (N - 1) + "]((int u, double e)) sortby u, groupby(2).\n" +
            // "ErrSum[int x:0..0](double s) groupby(1).\n" +
            // "DummyU(int i:0.." + (M - 1) + ").\n" +
            // "DummyV(int i:0.." + (N - 1) + ").\n" +
            "TmpU[int rank:0..4]((int u, (int l:0.."
            + (L - 1)
            + ", double p))) indexby u.\n"
            + "TmpV[int rank:0..4]((int v, (int l:0.."
            + (L - 1)
            + ", double q))) indexby v.\n"
            +
