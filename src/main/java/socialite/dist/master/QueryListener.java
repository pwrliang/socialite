...
public class QueryListener implements QueryProtocol {
  ...
  public BooleanWritable addToClassPath(Text hdfsPathText) {
    ...
    try {
      ...
      FileUtil.copy(hdfs, hdfsPath, localFs, localPath, false, true, hConf);
  ...
  public BooleanWritable removeFromClassPath(Text _file) {
    ...
    try {
      URL url = new File(localExtLibPath(file)).toURI().toURL();
  ...
  public void cleanupTableIter(LongWritable id) {
    try {
      Method cleanup =
          WorkerCmd.class.getMethod("cleanupTableIter", LongWritable.class);
  ...
  void runReally(Text prog, TupleSend qv) throws RemoteException {
    ...
    synchronized (this) {
      ...
      try {
        codeMain = en.compile(program, qv);
      } catch (Exception e) {
  ...
  @Override
  public void run(Text prog, Text clientIp, IntWritable port, LongWritable id)
      throws RemoteException {
    TupleSend queryVisitor =
        new TupleSend("QueryListener", clientIp.toString(), port.get(), id.get());
    runReally(prog, queryVisitor);
  }
  ...
  public void addPyFunctions(BytesWritable bytesClassFilesBlob, BytesWritable bytesPyfuncs)
    ...
    try {
      Method get =
          WorkerCmd.class.getMethod(
              "addPyFunctions", BytesWritable.class, BytesWritable.class);
  ...
  public void addClassFiles(BytesWritable bytesClassFilesBlob) throws RemoteException {
    ...
    Loader.loadFromBytes(classFilesBlob.names(), classFilesBlob.files());
    try {
      Method add = WorkerCmd.class.getMethod("addClassFiles", BytesWritable.class);
  ...
  public BytesWritable status(IntWritable _verbose) {
    ...
    try {
      Method status = WorkerCmd.class.getMethod("status", IntWritable.class);
    ...
    for (int i = 0; i < _workerStats.length; i++) {
      Status s = Status.fromWritable((BytesWritable) _workerStats[i]);
      workerStats[i] = s;
    }
  ...
  void memStatus(Status summary, WorkerAddrMap workerAddrMap, Status[] workerStats, int verbose) {
    ...
    if (verbose == 0) {
      ...
      summary.putMemStatus(allMemStat);
    }
  }

  void progressStatus(
      Status summary, WorkerAddrMap workerAddrMap, Status[] workerStats, int verbose) {
  ...
  public void runGc() {
    Method runGc = null;
    try {
      runGc = WorkerCmd.class.getMethod("runGc");
  ...
  public void setVerbose(BooleanWritable verbose) {
    ...
    try {
      Method setVerbose =
          WorkerCmd.class.getMethod("setVerbose", BooleanWritable.class);
  ...
  public void info() {
    try {
      Method info = WorkerCmd.class.getMethod("info");
      Object[] param = new Object[0];
