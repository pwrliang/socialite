...
public class Manager extends Thread {
  ...
  public static synchronized Manager getInst() {
    return theInstance;
  }
  ...
  JoinerBuilder builder(int rule) {
    ...
    JoinerBuilder builder = runtime.getJoinerBuilder(rule);
    if (builder == null) {
  ...
  int rand(int upto) {
    ...
    if (randval < 0) {
      randval = -randval;
      if (randval == Integer.MIN_VALUE) randval = 0;
    }
    return randval % upto;
  }
  ...
  public void addEvalCmd(EvalCommand evalCmd) throws InterruptedException {
    ...
    Task[] tasks;
    JoinerBuilder builder = builder(evalCmd.getRuleId());
    if (builder == null) {
      do {
        L.error(
            "Builder for rule#"
  ...
  void handleErrorLocal(Task t, Throwable e) {
    ...
    EvalRefCount.getInst().discard(t.getEpochId());
  }
