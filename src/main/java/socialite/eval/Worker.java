...
public class Worker implements Runnable {
  ...
  static {
    if (verbose) L.warn("Turn off the verbose flag in Worker.java");
  }
  ...
  public static TaskQueue[] getWorkerQueues() {
    return workerQueues;
  }

  static volatile boolean halted = false;

  public static void setHalted(boolean _halted) {
    halted = _halted;
  }
  ...
  public static void haltEpoch() {
    setHalted(true);
    Worker.emptyQueues();
    if (Sender.sendQ() != null) Sender.sendQ().empty();
    if (Receiver.recvq() != null) Receiver.recvq().empty();
  }
  ...
  void setRuntime(SRuntime _runtime) {
    runtime = _runtime;
  }

  RuleMap ruleMap(int rule) {
    return runtime.getRuleMap(rule);
  }

  JoinerBuilder builder(int rule) {
    return runtime.getJoinerBuilder(rule);
  }
  ...
  public void addTasksForDelta(int ruleid, TmpTableInst deltaT, Priority priority) {
    ...
    if (depRules != null) addTasksForDeltaReally(ruleid, deltaT, priority, depRules, true);
    if (depRulesRest != null) addTasksForDeltaReally(ruleid, deltaT, priority, depRulesRest, false);
  }

  void addTasksForDeltaReally(
      int ruleid, TmpTableInst deltaT, Priority priority, TIntArrayList depRules, boolean first) {
  ...
  void log(long start, long end, Task t) {
    int rid = t.getRuleId();
    String s =
        "=Worker " + id + "| Rule " + rid + "| start:" + start + "| end:" + end + "| task:" + t;
    log.add(s);
  }
