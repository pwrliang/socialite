...
import socialite.resource.TaskReport;
...
abstract class VisitorWrapper extends Joiner {
  ...
  public TableInst[] getDeltaTables() {
    return v.getDeltaTables();
  }

  public TableInst[] getPrivateTable() {
    return v.getPrivateTable();
  }

  public void run() {
    v.run();
    int count = counter.incrementAndGet();
    if (count == limit) cleanup();
  }
...
class VisitorReportingProgress extends VisitorWrapper {
  ...
  public VisitorReportingProgress(
      TaskReport _report, Joiner _v, AtomicInteger _counter, int _limit) {
    super(_v, _counter, _limit);
    report = _report;
...
public class TaskFactory {
  ...
  static boolean isNull(TableInst[] tableArray) {
    ...
    for (int i = 0; i < tableArray.length; i++) {
      if (tableArray[i] != null) return false;
    }
    return true;
  }
  ...
  public EvalTask[] make(int ruleid, TableInst[] tableArray, JoinerBuilder builder) {
    ...
    if (requireFree(tableArray)) joiners = addTableFree(joiners, tableArray);
  ...
  boolean requireFree(TableInst[] tableArray) {
    ...
    for (int i = 0; i < tableArray.length; i++) {
      if (tableArray[i] != null && tableArray[i] instanceof TmpTableInst) return true;
    }
    return false;
  }
  ...
  static Joiner[] addTaskReport(Joiner[] v, SRuntime runtime) {
    ...
    for (int i = 0; i < v.length; i++) {
      if (v[i] == null) {
        ...
        wrappers[i] = new VisitorReportingProgress(taskReport, v[i], counter, limit);
      }
    }
    counter.set(initCounter);
    return wrappers;
