...
public class DeltaStepWindow {
  ...
  public static void reset() {
    ...
    for (DeltaStepWindow w : allWindows) w._reset();
  }
...
class IntWindow {
  ...
  public String toString() {
    ...
    for (int i = 0; i < boundaries.length; i++) str += boundaries[i] + " ";
    return str + "]";
  }
  ...
  void updateBoundaries() {
    ...
    for (int i = 0; i < DeltaStepWindow.maxLevel; i++) boundaries[i] = myMin + delta * (i + 1);
  }
...
class FloatWindow {
  ...
  public int selectLevel(float v, boolean min) {
    ...
    if (v > myMax) {
      if (myMax - myMin < (v - myMax) * 2) level = 0;
      myMax = v;
      updated = true;
    }
