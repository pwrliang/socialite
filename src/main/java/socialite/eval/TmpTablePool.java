...
public class TmpTablePool {
  ...
  static Map<Class, Method> tableAlloc =
      Collections.synchronizedMap(new WeakHashMap<Class, Method>());
  static Map<Class, Method> tableAllocSmall =
      Collections.synchronizedMap(new WeakHashMap<Class, Method>());
  ...
  static int tableid(Class tableCls) {
    try {
      Method getId = tableCls.getMethod("tableid");
  ...
  static WeakArrayQueue<TmpTableInst> getQueue(
      Map<Class, WeakArrayQueue<TmpTableInst>> freeTableListMap,
      Class tableCls,
      int queueInitSize) {
  ...
  static TmpTableInst get_global(int urgency, Class tableCls, Object... args) {
    ...
    try {
      do {
        ...
        if (freeMem > 1024 * 1024 * (1024 + 1024 + 512)) {
          break;
        }
        if (urgency >= 1 && freeMem > 1024 * 1024 * (1024 + 512 + 256)) {
          break;
        }
        if (urgency >= 2 && freeMem > 1024 * 1024 * (1024 + 512)) {
          break;
        }
        if (tryCnt > maxTry) {
          break;
  ...
  static TmpTableInst alloc(Class tableCls, Object... args) {
    try {
      ...
      TmpTableInst t = (TmpTableInst) alloc.invoke(null, args);
