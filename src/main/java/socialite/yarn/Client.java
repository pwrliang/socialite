...
public class Client {
  ...
  List<ApplicationReport> getRunningAppMaster(YarnClient yarnClient)
      throws IOException, YarnException {
    HashSet<String> appTypes = new HashSet();
    appTypes.add(yarnAppType);
    EnumSet<YarnApplicationState> appStats =
        EnumSet.of(
  ...
  public boolean killAppMaster() throws Exception {
    ...
    if (appReports.size() > 1) {
      System.out.println(
          "[WARN] Multiple Socialite AppMaster is not running! # " + appReports.size());
    }
    ...
    while (true) {
      ...
      if (appState == YarnApplicationState.RUNNING) {
        ...
        throw new RuntimeException(
            "Failed to launch Socialite AppMaster. YarnApplicationState:" + appState);
      }
    }
  }
  ...
  void setupAppMasterJar(Path localJarPath, LocalResource appMasterJar) {
    try {
      FileSystem fs = FileSystem.get(new Configuration());
      Path jarPath =
          getHdfsJarPath(String.format("socialite-all-%d.jar", System.currentTimeMillis() / 1000));
