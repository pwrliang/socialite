...
public class Sender {
  ...
  public Sender(WorkerAddrMap _workerAddrMap, WorkerConnPool _workerConn) {
    init(_workerAddrMap, _workerConn);
  }

  public void init(WorkerAddrMap _workerAddrMap, WorkerConnPool _workerConn) {
    ...
    if (sendChannel$[machineIdx] == null) {
      ...
      if (sendChMux == null) {
        throw new SociaLiteException(
            "SendChannel is null for machine:" + machineIdx + ", workerAddr:" + sockaddr);
      }
    }
  }
  ...
  public void run() {
    while (true) {
      ...
      try {
        ...
        sendQ.pop(m);
        if (buffer != null) ByteBufferPool.get().free(buffer);
      }
    }
  }
}
