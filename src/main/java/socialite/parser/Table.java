...
public class Table implements Serializable {
  ...
  public void setVisitorClass(String _visitorClass) {
    visitorClass = _visitorClass;
  }
  ...
  public void setGroupByColNum(int num) throws InternalException {
    if (groupby > 0 && groupby != num) {
      throw new InternalException(
          "Table " + name() + " is already configured with #" + groupby + " groupby columns.");
    }
  ...
  public boolean hasNesting() {
    return !nestedTableIndices.isEmpty();
  }
  ...
  public boolean nestingBegins(int col) {
    return nestedTableIndices.contains(col);
  }

  public int[] sortbyCols() {
    if (sortbyCols == null) {
      sortbyCols = decl.sortbyCols().toArray();
    }
    return sortbyCols;
  }
