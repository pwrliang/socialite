...
public class AssignOp extends Op {
  ...
  public boolean fromFunction() {
    return arg2 instanceof Function;
  }
  ...
  static Class<?> getNonPrimOrStr(@SuppressWarnings("rawtypes") Set<Class> types) {
    for (Class<?> type : types) {
      if (!(MyType.isPrimitive(type) || type.equals(String.class))) return type;
    }
    return null;
  }
  ...
  static boolean containsOnlyPrimOrStr(@SuppressWarnings("rawtypes") Set<Class> types) {
    for (Class<?> type : types) {
      if (!(MyType.isPrimitive(type) || type.equals(String.class))) return false;
    }
    return true;
  }
    ...
    if (multiRows()) {
      ...
      if (isMultipleAssign() && f.isArrayType()) {
        ...
        if (MyType.isPrimNumberType(lhs.type)) {
          ...
          stmts.add(
              "stmts",
              arg1 + "=(" + pyToJava + ")((Number)" + f.codegen().render() + ")" + valueGetter);
        } else {
  ...
  public boolean isMultipleAssign() {
    return arg1 instanceof List;
  }
  ...
  public void getRhsTypes(@SuppressWarnings("rawtypes") Collection<Class> types)
  ...
  public Set<Variable> getLhsVars() {
    ...
    if (arg1 instanceof Variable) {
      ...
      for (Variable v : (List<Variable>) arg1) vars.add(v);
    }
    return vars;
  }
  ...
  public void visit(OpVisitor v) {
    v.visit(this);
    if (arg2 instanceof Op) v.visit((Op) arg2);
  }
