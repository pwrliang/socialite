...
public class ColumnDecl implements Externalizable {
  ...
  public static int guessRowSizeFromTypes(List<Class> types) {
    assert types != null;
    int size = 0;
    for (Class t : types) size += getTypeSize(t);
    return size;
  }
  ...
  public ColumnDecl(Class _type, String _name, int _pos) {
    this(_type, _name, null);
    pos = _pos;
  }
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    for (int i = 0; i < _typename.length; i++) _typename[i] = in.readChar();
