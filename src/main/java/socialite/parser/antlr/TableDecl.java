...
public class TableDecl implements Serializable {
  ...
  public void checkTypes(List<?> params) throws InternalException {
    ...
    for (int i = 0, j = 0; i < params.size(); i++) {
      ...
      if (iterColumn() == i) {
        ...
        columnType = allColDecls.get(j++).type();
      }
  ...
  public String toString() {
    ...
    if (nestedTable != null) s += ",(" + nestedTable + ")";
    s += ")";
  ...
  public boolean equals(Object o) {
    ...
    if (nestedTable != null && !nestedTable.equals(td.nestedTable)) return false;
  ...
  public TIntArrayList getNestedTableIndices() {
    ...
    if (nestedTable != null) {
      ...
      for (ColumnDecl decl : colDecls) {
        if (decl.option() instanceof ColIter) nestedIdx--;
      }
  ...
  void setColumnPositions() {
    ...
    if (nestedTable != null) nestedTable.setColumnPosition(pos, colNameToPos);
  }

  public int getColumnPos(String name) {
    if (colNameToPos.contains(name)) return colNameToPos.get(name);
    return -1;
  }

  public boolean hasIterColumn() {
    return iterCol >= 0;
  }

  public int iterColumn() {
    return iterCol;
  }
  ...
  public TIntArrayList sortbyCols() {
    return columnsWithOption(sortBy);
  }

  public TIntArrayList indexbyCols() {
    return columnsWithOption(indexBy);
  }
  ...
  int numColumns() {
    ...
    if (nestedTable != null) num += nestedTable.numColumns();
    return num;
  }
      ...
      if (opt instanceof TemplateType) {
        ...
        if (partitionCol != 0 && colOpt instanceof ColRange) {
          throw new ParseException(
              "Cannot use shardby for a column with a range annotation, except for a first column.");
        }
        ...
        if (gb <= 0)
          throw new ParseException(
              "groupby argument should be greater than 0 (1 if iteration column is used).");
        groupby = gb;
