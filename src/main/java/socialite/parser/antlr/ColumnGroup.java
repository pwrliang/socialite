...
public class ColumnGroup implements Externalizable {
  ...
  public int hashCode() {
    int h = 0;
    for (Column c : columns) h ^= c.hashCode();
    return h;
  }
  ...
  public boolean equals(Object o) {
    ...
    for (int i = 0; i < columns.length; i++) {
      if (!columns[i].equals(group.columns[i])) return false;
    }
    return true;
  }
  ...
  public int startIdx() {
    return columns[0].position();
  }

  public int endIdx() {
    return startIdx() + columns.length - 1;
  }

  public Column getSortedCol() {
  ...
  public boolean isSorted() {
    return getSortedCol() != null;
  }

  public boolean hasArrayIndex() {
    return columns[0].isArrayIndex();
  }
  ...
  public Class[] types() {
    if (types == null) {
      ...
      for (int i = 0; i < types.length; i++) types[i] = columns[i].type();
    }
    return types;
  }
  ...
  public boolean hasBitPackedColumn() {
    for (Column c : columns) {
      if (c.hasNumBits()) {
        return true;
      }
