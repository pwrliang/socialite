...
public class NestedTableDecl implements Externalizable {
  ...
  public ColumnDecl first() {
    return colDecls.get(0);
  }

  public ColumnDecl last() {
    if (nestedTable != null) return nestedTable.last();
  ...
  public NestedTableDecl clone() {
    ...
    if (nestedTable != null) nestedClone = nestedTable.clone();
  ...
  public boolean equals(Object o) {
    ...
    if (nestedTable != null) return nestedTable.equals(d.nestedTable);
    else return true;
  }
  ...
  public void setColumnPosition(int start, TObjectIntHashMap<String> colNameToPos) {
    ...
    for (; i < colDecls.size(); i++) {
      ...
      d.setPos(i + start);
      if (colNameToPos.contains(d.name)) Assert.die("Column name " + d.name + " is already used");
      colNameToPos.put(d.name, i + start);
    }
    if (nestedTable != null) nestedTable.setColumnPosition(start + i, colNameToPos);
  }

  public void colDecls(List<ColumnDecl> decls) {
    decls.addAll(colDecls);
    if (nestedTable != null) nestedTable.colDecls(decls);
  }
