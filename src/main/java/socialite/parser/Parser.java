...
/**
 * Parser instance keeps track of declared tables to check if predicates confirms to the table
 * declarations.
 *
 * @author jiwon
 */
public class Parser {
  ...
  public void parse() {
    ...
    for (Object decl : decls) {
      if (decl instanceof TableDecl) {
        ...
        if (queryNum++ > 0) throw new SociaLiteException("Cannot have multiple query statements.");
  ...
  boolean isIntConst(Object o) {
    if (o instanceof Const) {
      return MyType.javaType(o).equals(Integer.class) || MyType.javaType(o).equals(int.class);
    }
    return false;
  }
  ...
  int removeIterColumn(Predicate p) {
    ...
    if (!isIntConst(iterParam)) {
      String msg =
          "Integer constant is required for the iteration column ("
              + iterCol
              + "'th column) of "
              + p.name();
