...
public class Function implements Param {
  ...
  public Set<Variable> getVariables() {
    ...
    for (Object o : getArgs()) {
      if (o instanceof Variable) vars.add((Variable) o);
    }
    return vars;
  }
  ...
  public boolean isInRuleHead() {
    return false;
  }

  public boolean isLoaded() {
    return method != null || pyfunc != null;
  }
  ...
  public boolean multiRows() {
    if (method != null) {
      ...
      return Iterator.class.isAssignableFrom(type) || TIterator.class.isAssignableFrom(type);
    } else {
      CompilerFlags flags = ((PyBaseCode) pyfunc.__code__).co_flags;
  ...
  void loadPyFunc() throws InternalException {
    ...
    if (pyfunc.__defaults__ != null && pyfunc.__defaults__.length > 0)
      throw new InternalException("Default values for args are not supported:$" + pyname);

    CompilerFlags f = ((PyBaseCode) pyfunc.__code__).co_flags;
    ...
    if (!(pyfunc.__code__ instanceof PyBaseCode)) {
      throw new InternalException(
          "Unexpected __code__ type("
              + pyfunc.__code__.getClass().getSimpleName()
              + ") for "
              + pyname);
    }
  ...
  Class<?> getMethodReturnType() {
    if (method != null) {
      ...
      assert pyfunc != null;
      PyStringMap fmap = (PyStringMap) pyfunc.getDict();
  ...
  Class<?> doTypeCast(TypeCast cast, Class<?> fromType) throws InternalException {
    ...
    if (!cast.isValid(fromType)) {
      String msg =
          "Unexpected type cast from "
              + fromType.getSimpleName()
              + " to "
              + cast.type.getSimpleName();
  ...
  void computeJavaArrayReturnVarTypes(TypeCast cast, Class<?> methodRetType)
    ...
    if (getReturns().size() == 1) {
      ...
      if (types == null) {
        ...
        if (cast != null)
          throw new InternalException("Unexpected type cast to " + cast.type.getSimpleName());
  ...
  void computePyReturnVarTypes(TypeCast cast) throws InternalException {
    ...
    if (pytypes instanceof PyType) {
      ...
      if (types.length != getReturns().size()) throw new InternalException(msg);
      for (int i = 0; i < types.length; i++) {
        getReturns().get(i).setType(types[i]);
      }
    }
  ...
  void getPyReturnTypesForTuple() throws InternalException {
    ...
    for (int i = 0; i < types.length; i++) getReturns().get(i).setType(types[i]);
  }
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    for (int i = 0; i < len; i++) _name[i] = in.readChar();
  ...
  String maybeAddBuiltin(String _name) {
    if (_name.indexOf('.') >= 0) {
      return _name;
    } else {
