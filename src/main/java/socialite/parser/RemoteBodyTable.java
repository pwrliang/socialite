...
/**
 * Table class representing remote rule body e.g. Triange(0, $inc(1)) :- Edge(a,b), Edge(b,c),
 * Edge(c,a). For the first and second join operation in the rule body, we perform distributed join.
 * The system automatically transfers and stores the data in "remote body tables", and adds extra
 * rules as following. Triange(0, $inc(1)) :- RemoteEdge(a,b), Edge(b,c), Edge(c,a). Triange(0,
 * $inc(1)) :- RemoteEdge(a,b,c), Edge(c,a). where the join operation betweem RemoteEdge to the next
 * table is always local join operation.
 */
public class RemoteBodyTable extends Table implements GeneratedT {
  ...
  static TableDecl genDecl(String name, List<List<Class>> nestedTypes) {
    if (nestedTypes.size() > 4) {
      ...
      for (int i = 4; i < nestedTypes.size(); i++) lastCol.addAll(nestedTypes.get(i));
      int removeCnt = nestedTypes.size() - 4;
      for (int i = 0; i < removeCnt; i++) nestedTypes.remove(4);
    }
