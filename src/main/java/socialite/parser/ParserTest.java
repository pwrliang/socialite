...
public class ParserTest {
  ...
  static void testTableReadWrite() {
    String query =
        "PageRank(int n,int _iter, double r).\n"
            + "Edge(int s,int t).\n"
            + "PageRank(t,0, r) :- Edge(p,t), r = 1.0/100.\n";
  ...
  static void testAggrFunc() {
    ...
    query =
        "Bar(int a:0..100, (int i, double d)). \n" + "Bar(a, $min(b), c) :- a=1, b=20, c=1.1.\n";
    p.parse(query);
    an = new Analysis(p);
    try {
      an.run();
  ...
  public static void main(String[] args) {
    ...
    testSyntaxErrorWithNonConstantIter();
    dot();
