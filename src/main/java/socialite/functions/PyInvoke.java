  ...
  static void init(int funcId, PyFunction func) {
    ...
    for (int i = 0; i < workernum; i++) {
      if (states.get(i) == null) {
        ThreadState ts = new ThreadState(PyInterp.get().getSystemState());
        states.set(i, ts);
      }
    }
    ...
    for (int i = 0; i < workernum; i++) {
      ...
      if (frame == null) {
        frame = new PyFrame((PyBaseCode) func.__code__, func.__globals__);
...
class PyFunctionCache {
  ...
  static synchronized int get(String name) {
    if (!nameToIdx.contains(name)) return -1;
    return nameToIdx.get(name);
  }

  static synchronized int get(PyFunction pyf) {
    if (!funcToIdx.contains(pyf)) return -1;
...
public class PyInvoke {
  ...
  public static void update(List<PyFunction> pyfuncs) {
    for (PyFunction pyfunc : pyfuncs) update(pyfunc);
  }
  ...
  public static Object invoke(int workerId, int funcIdx) {
    ...
    PyBaseCode code = (PyBaseCode) func.__code__;
    if (isGenerator(code)) {
      frame = new PyFrame((PyBaseCode) func.__code__, func.__globals__);
      PyGenerator pygen = new PyGenerator(frame, func.__closure__);
      return new PyGenIterator(pygen);
    }
    PyObject ret = code.call(ts, frame, func.__closure__);
    return toJava(ret);
  }
  ...
  public static Object invoke(
      int workerId, int funcIdx, Object arg0, Object arg1, Object arg2, Object arg3) {
    PyFunction func = PyFunctionCache.get(funcIdx);
    ThreadState ts = PyStateCache.get(workerId);
    PyFrame frame = PyStateCache.getFrame(workerId, funcIdx);
    frame.f_fastlocals[0] = Py.java2py(arg0);
  ...
  static class PyGenIterator implements Iterator<Object> {
    ...
    public boolean hasNext() {
      try {
        ...
        if (e.type.equals(Py.StopIteration)) return false;
        throw e;
      }
    }
