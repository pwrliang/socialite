...
public class Epoch implements Externalizable {
  ...
  TIntObjectHashMap<String> visitorClassMap =
      new TIntObjectHashMap<String>(); // rule-id to visitor class
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    for (int i = 0; i < _name.length; i++) _name[i] = in.readChar();
  ...
  public List<TableStmt> tableStmts() {
    return tableStmts;
  }
  ...
  public void setRuleMap(RuleMap _map) {
    ruleMap = _map;
  }
  ...
  public List<RuleComp> getRuleComps() {
    return ruleComps;
  }
  ...
  public boolean hasInitRule() {
    for (Rule r : getRules()) {
      if (r.isSimpleArrayInit()) return true;
    }
    return false;
  }

  public void addVisitorClass(int rule, Class<?> visitorClass) {
    assert !visitorClassMap.contains(rule)
        : "rule[" + rule + "] already has visitor:" + visitorClass;
    visitorClassMap.put(rule, visitorClass.getName());
  }

  public String getVisitorClassName(int rule) {
    if (visitorClassMap.contains(rule)) return visitorClassMap.get(rule);
    return null;
  }
  ...
  static SArrayList<RuleComp> topologicalSort(List<RuleComp> _ruleComps) {
    ...
    for (RuleComp rc : _ruleComps) {
      if (rc.getRuleCompsUsingThis().size() == 0) initSet.add(rc);
    }

    for (RuleComp rc : initSet) dfsVisit(rc, visited, sorted);
    ...
    while (it.hasNext()) {
      RuleComp rc = it.next();
      if (!_ruleComps.contains(rc)) it.remove();
    }
  ...
  static void removeSimpleAssignRules(List<RuleComp> list) {
    ...
    while (it.hasNext()) {
      RuleComp rc = it.next();
      if (rc.size() == 1 && rc.get(0).isSimpleArrayInit()) it.remove();
    }
  }
  ...
  static void dfsVisit(RuleComp rc, Set<RuleComp> visited, SArrayList<RuleComp> sorted) {
    ...
    visited.add(rc);
    for (RuleComp dep : rc.getDependingRuleComps()) dfsVisit(dep, visited, sorted);
    sorted.add(rc);
  }
}
