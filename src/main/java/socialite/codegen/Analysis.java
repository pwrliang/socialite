...
public class Analysis {
  ...
  protected Analysis(
      List<Rule> _rules,
      List<Table> _newTables,
      Map<String, Table> _tableMap,
      List<TableStmt> _tableStmts,
      Query _query) {
    rules = _rules;
  ...
  public Analysis(Parser _p) {
    this(_p.getRules(), _p.getNewTables(), _p.getTableMap(), _p.getTableStmts(), _p.getQuery());
    p = _p;
  }
  ...
  public List<Table> getDeltaTables() {
    return deltaTables;
  }

  public List<Table> getRemoteTables() {
    return remoteTables;
  }

  public List<Table> getPrivateTables() {
    return privTables;
  }

  public Map<String, Table> getTableMap() {
    return new HashMap<String, Table>(tableMap);
  }
    ...
    for (Table t : newTables) {
      ...
      for (int col : t.indexedCols()) {
        if (col == groupby) {
          ...
          String msg =
              "Cannot add indexby to groupby target column (column "
                  + c.name()
                  + " in table "
                  + t.name()
                  + ")";
  ...
  void runTypeCheck() {
    ...
    while (requireTypeChecking()) {
      if (FunctionLoader.allLoaded(rules)) {
        computeParamTypes();
        if (requireTypeChecking()) throwUnresolvedTypeException();
      }
      computeParamTypes();
      FunctionLoader.loadAll(rules);
        ...
        for (Variable v : vars) {
          if (assigned.contains(v)) {
            String msg =
                "Variable "
                    + v.name
                    + " is already assigned a value, so cannot be reassigned in "
                    + e;
            throw new AnalysisException(msg, r);
          }
        }
  ...
  public static boolean canMatch(Predicate p1, Predicate p2, Map<String, Table> map) {
    ...
    if (p1.hasFunctionParam()) compareUpto = p1.firstFunctionIdx();
    if (p2.hasFunctionParam()) compareUpto = p2.firstFunctionIdx();
    ...
    for (int i = 0; i < compareUpto; i++) {
      if (params1[i] instanceof Const && params2[i] instanceof Const) {
        ...
        if (!c1.val.equals(c2.val)) return false;
      }
    }
    return true;
  }
  ...
  boolean canMatchAny(Predicate p1, List<Predicate> plist) {
    for (Predicate p2 : plist) {
      if (canMatch(p1, p2)) return true;
    }
    return false;
  }

  /**
   * returns true if the updating shard at the head table is same as the block at the first table.
   */
  ...
  static boolean isHashShardedColumn(Column c1, Column c2) {
    return !c1.hasRange() && !c2.hasRange();
  }
  ...
  List copyParams(List params) {
    ...
    for (Object o : params) {
      if (o instanceof AggrFunction) {
        ...
        for (Param a : f.getArgs()) args.add(a);
  ...
  public static List<Variable> collectLiveVarsAt(Rule r, int pos) {
    ...
    while (i.hasNext()) {
      if (!usedVars.contains(i.next())) i.remove();
    }
    return liveVars;
  }
  ...
  List<Variable> topologicalSort(List<Variable> relatedVars, Map<Object, Set> oneToMany) {
    ...
    for (Variable v : initSet) dfsVisit(v, oneToMany, visited, sorted);
    return sorted;
  }

  @SuppressWarnings("rawtypes")
  void dfsVisit(
      Variable v, Map<Object, Set> oneToMany, Set<Variable> visited, List<Variable> sorted) {
  ...
  Map<Object, Set> getOneToManyRelation(Rule r, int transferPos) {
    if (oneToManyMap.containsKey(r)) return oneToManyMap.get(r);
    ...
    for (Predicate p : r.getBodyP()) {
      ...
      for (int pos : t.nestPos()) {
        for (int i = prevPos; i < pos; i++) {
          if (!(p.params.get(i) instanceof Variable)) continue;
  ...
  void processRemoteRuleBody(Rule r, List<Rule> toAdd) {
    ...
    for (int pos : sendPos) {
      ...
      // add new rule
      SArrayList<Param> x = new SArrayList<Param>();
      x.addAll(vars);
  ...
  boolean hasDelta(Rule r) {
    return r.inScc() || r.hasPipelinedRules();
  }
  ...
  List makeNewParams(List<Param> oldParams, List<Param> input) {
    ...
    for (Param o : oldParams) {
      if (o instanceof AggrFunction) {
        ...
        for (Object a : f.getArgs()) args.add(input.get(i++));
        AggrFunction aggr = new AggrFunction(f, args);
        newParam.add(aggr);
      } else {
  ...
  List replaceInputWithVars(List params, List<Variable> vars) {
    ...
    for (Object o : params) {
      if (o instanceof AggrFunction) {
        AggrFunction f = (AggrFunction) o;
        List args = new ArrayList();
        for (Object a : f.getArgs()) args.add(vars.get(i++));
        AggrFunction aggr = new AggrFunction(f, args);
        replaced.add(aggr);
      } else replaced.add(vars.get(i++));
    }
    return replaced;
  }
  ...
  List<Predicate> getMatchingPs(Rule r, String name) {
    ...
    for (Predicate p : r.getBodyP()) {
      if (name.equals(p.name())) list.add(p);
    }
    return list;
  }
        ...
        for (Rule r2 : depRules) {
          ...
          for (Predicate p : getMatchingPs(r, name)) {
            ...
            boolean existingRule =
                tableMap.containsKey(DeltaTable.name(p))
  ...
  boolean requireTypeChecking(Rule r) {
    ...
    return typedVars != r.getBodyVariables().size();
  }

  public boolean requireTypeChecking() {
    for (Rule r : rules) {
      if (requireTypeChecking(r)) return true;
    }
    return false;
  }
  ...
  int countTypedVars(Set<Variable> vars) {
    int typedVars = 0;
    for (Variable v : vars) {
      if (v.hasType() /* || v.dontCare*/) typedVars++;
    }
    return typedVars;
  }
    ...
    for (Rule r : rules) {
      ...
      for (Predicate p : r.getBodyP()) {
        if (p.hasFunctionParam()) {
          String msg =
              "Functions cannot be in a parameter list of a predicate except for head predicate";
          throw new AnalysisException(msg, r);
        }
      }
    }
  ...
  boolean appearsInAnyRuleBody(Table t) {
    for (Rule rule : rules) {
      if (appearsInRuleBody(t, rule)) return true;
    }
    return false;
  }
  ...
  boolean findScc(
      Table root, Map<Table, List<Table>> deps, Table cur, Set<Table> path, Set<Table> visited) {
  ...
  List<RuleComp> findRuleComp() {
    ...
    for (Rule r : rules) {
      if (!addedRules.contains(r)) comps.add(new RuleComp(r));
    }
    return comps;
  }
  ...
  boolean hasNoMatchButNegatedOne(Rule r, Predicate p) {
    for (Predicate p2 : r.getBodyP()) {
      ...
      if (p2.name().equals(p.name())) return false;
    }
    return true;
  }
  ...
  List<Rule> findSCCfrom(Rule root, Set<Rule> marked, Rule currentRule) {
    ...
    for (Rule r2 : currentRule.getRulesUsingThis()) {
      ...
      if (r2 == root && canMatchAny(currentRule.getHead(), r2.getBodyP())) {
        ...
        result.add(r2);
        if (!result.contains(currentRule)) result.add(currentRule);
        return result;
      }
      ...
      if (scc != null) {
        if (!scc.contains(currentRule)) scc.add(currentRule);
        return scc;
      }
    }
    return null;
  ...
  boolean updateEpochLevel(List<RuleComp> rclist, TObjectIntHashMap<RuleComp> epoch, int l) {
    ...
    for (RuleComp rc : rclist) {
      if (updateEpochLevel(rc, epoch, l)) change = true;
    }
    return change;
  }
  ...
  void assertSameLevel(List<RuleComp> ruleComps, TObjectIntHashMap<RuleComp> stratum) {
    RuleComp rc = ruleComps.get(0);
    int l = stratum.get(rc);
    for (RuleComp rc2 : ruleComps) {
      assert l == stratum.get(rc2) : "RuleComp[" + rc2 + "]" + "'s level:" + stratum.get(rc2) + "!=" + l;
    }
  }
  ...
  void makeEpochs() {
    ...
    for (RuleComp rc : ruleComps) levelMap.put(rc, 0);
    ...
    do {
      ...
      for (RuleComp rc : ruleComps) {
        ...
        List<RuleComp> usedBy = new ArrayList<RuleComp>(rc.getRuleCompsUsingThis());
        usedBy.remove(rc);
    ...
    for (int i = 0; i <= highest; i++) {
      ...
      for (RuleComp rc : ruleComps) {
        int l = levelMap.get(rc);
        if (l == i) ruleCompsInLevel.add(rc);
      }
  ...
  void computeExprTypes(Rule r) {
    ...
    for (Object o : r.getBody()) {
      if (o instanceof Expr) forceTypesForAssignOp((Expr) o);
    }
  }
  ...
  boolean containsOnlyPrim(Set<Class> types) {
    for (Class type : types) {
      if (!MyType.isPrimitive(type)) return false;
    }
    return true;
  }
    ...
    for (Rule r : rules) {
      ...
      for (int i = 0; i < params.length; i++) {
        ...
        if (p instanceof Variable) {
          ...
          if (v.dontCare && !c.isArrayIndex()) continue nextRule;
        }
      }
  ...
  public static boolean isResolved(Rule rule, Predicate p, Object param) {
    if (param instanceof Variable) return isResolved(rule, p, (Variable) param);
