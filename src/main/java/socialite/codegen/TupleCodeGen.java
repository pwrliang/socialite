...
public class TupleCodeGen {
  ...
  public String name() {
    if (tupleClassName == null) {
      ...
      for (int i = 0; i < types.length; i++) {
        if (MyType.isPrimitive(types[i])) tupleNameTmpl.add("types", MyType.javaTypeName(types[i]));
        else tupleNameTmpl.add("types", "Object");
      }
  ...
  public String generate() {
    for (int i = 0; i < types.length; i++) {
      if (MyType.isPrimitive(types[i])) tupleTmpl.add("types", MyType.javaTypeName(types[i]));
      else tupleTmpl.add("types", "Object");
    }
  ...
  public static void main(String[] args) {
    String query =
        "Foo(int s:0..1, String s2, int d, double d2). \n"
  ...
  static final int PREGEN_WIDTH = 3; // Max # of columns for pre-generated tuples

  public static LinkedHashMap<String, byte[]> generate(
      List<Rule> rules, Map<String, Table> tableMap) {
    Compiler c = new Compiler();
    TupleDecls toGen = new TupleDecls();
    for (Rule r : rules) {
      findTuplesToGen(r, tableMap, toGen);
    }
    if (toGen.isEmpty()) return new LinkedHashMap<String, byte[]>(0);
    ...
    for (TupleDecl tupDecl : toGen) {
      ...
      if (Loader.exists(tgen.name())) continue;

      boolean success = c.compile(tgen.name(), tgen.generate());
      if (!success) {
        String msg = "compilation error for " + tgen.name() + " " + c.getErrorMsg();
  ...
  static void findTuplesToGen(Rule rule, Map<String, Table> tableMap, TupleDecls toGen) {
  ...
  static void _findTuplesToGenForIterTable(Table t, TupleDecls required) {
    if (t instanceof IterTableMixin && t.numColumns() + 1 > PREGEN_WIDTH) {
      ...
      for (int i = 0; i < t.numColumns(); i++) types.add(t.getColumn(i).type());
...
class TupleDecls implements Iterable<TupleDecl> {
  ...
  Set<TupleDecl> tupleTypes() {
    if (tupleDecls == null) tupleDecls = new LinkedHashSet<TupleDecl>();
    return tupleDecls;
  }

  public boolean isEmpty() {
    if (tupleDecls == null) return true;
...
class TupleDecl {
  ...
  public int hashCode() {
    int h = 0;
    for (Class<?> t : types) h ^= t.hashCode();
    return h;
  }

  public boolean equals(Object o) {
    if (!(o instanceof TupleDecl)) return false;

    TupleDecl t = (TupleDecl) o;
    if (types.length != t.types.length) return false;
