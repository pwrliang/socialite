...
public class CodeGenBase {
  ...
  static {
    ...
    combinedIterateGroup =
        new MySTGroupFile(JoinerCodeGen.class.getResource(combinedIterateGroupFile));
  ...
  public static ST expr() {
    return getVisitorST("expr");
  }

  public static ST stmts() {
    return getVisitorST("simpleStmts");
  }
  ...
  public static void fillVisitMethodBody(
      ST method, Predicate p, int startCol, int endCol, Collection<Variable> resolved) {
    Object[] params = p.inputParams();
    for (int i = startCol; i <= endCol; ) {
      String arg = "_" + (i - startCol); // same as addArgTypes
      if (params[i] instanceof Variable && ((Variable) params[i]).dontCare) {
        i++;
        continue;
      }
