...
public class JoinerCodeGen {
  ...
  void generateConstructor() {
    ...
    for (Predicate p : rule.getAllP()) {
      ...
      code.add(
          "stmts", init(var, cast(tableClass) + call(registryVar(), "getTableInstArray", t.id())));
    }
  ...
  void genGetDeltaTableReallyWithPriority(
      ST method, String deltaVar, String deltaRetVar, String priority) {
    ...
    if (hasPipelinedRules()) {
      ...
      if_.add(
          "stmts",
          cond + " getWorker().addTasksForDelta(getRuleId()," + deltaVar + ", " + priority + ")");
      if_.add("stmts", deltaVar + "=" + alloc);
      if_.add("stmts", deltaRetVar + "=" + deltaVar + ".isEmpty()?" + deltaVar + ":null");
    }
  ...
  void addPipeliningCode(ST code, String deltaVar) {
    String deltaRulesArray =
        runtimeVar() + ".getRuleMap(getRuleId()).getDeltaRules(getRuleId()).toArray()";
    ...
    String visitors =
        runtimeVar()
            + ".getJoinerBuilder(_$pr).getNewJoinerInst(_$pr, new TableInst[]{"
            + deltaVar
            + "})";
  ...
  boolean isRemoteHeadFirstT() {
    ...
    return t instanceof RemoteHeadTable;
  }

  String remoteTable0MapKey(int tableId, String machineIdx) {
    return "((((long)" + machineIdx + ")<<30) |" + "(((long)" + tableId + ")<<1)" + "|0)";
  }

  String remoteTableMapKey(int tableId, String machineIdx) {
    return "((((long)" + machineIdx + ")<<30) |" + "(((long)" + tableId + ")<<1)" + "|1)";
  }
  ...
  DeltaPredicate getFirstPifDelta() {
    if (rule.firstP() instanceof DeltaPredicate) return (DeltaPredicate) rule.firstP();
    return null;
    /*
  ...
  void maybeSendToRemoteHead(ST code, String table, String machineIdx, Object idxParam) {
    ST if_ = tmplGroup.getInstanceOf("if");
    code.add("stmts", if_);
  ...
  void sendToRemoteHead(ST code, String table, String partitionIdx, String machineIdx) {
    ...
    code.add(
        "stmts",
        "EvalWithTable _$cmd=new EvalWithTable("
            + epochIdVar()
            + ", _$depRuleId,"
            + table
            + ","
  ...
  void maybeSendToRemoteBody(ST code, String table, Predicate joinP) {
    ...
    ST send = if_;
    String machineIdx =
        partitionMapVar() + ".machineIndexFor(" + t.id() + "," + joinP.first() + ")";
  ...
  void broadcastToRemoteBody(ST code, String table, String pos) {
    ...
    code.add(
        "stmts",
        "EvalWithTable _$cmd=new EvalWithTable(" + epochIdVar() + ",_$depRuleId," + table + ",0)");
  ...
  void generateRunMethod() {
    ...
    for (Object o : rule.getBody()) {
      if (o instanceof Expr) {
        ...
        assert false : "Expecting Expr or Predicate, but got:" + o;
      }
    }
    if (iterStartP == null) {
  ...
  boolean isIndexColResolved(Predicate p) {
    return getResolvedIndexCols(p).size() != 0;
  }
  ...
  CmpOp.CmpType cmpTypeForIteratePart(Predicate p) {
    ...
    if (val.equals(rhs)) return cmpOp.cmpType();
    else return cmpOp.cmpType().reverse();
  }
  ...
  int idxForIteratePart(Predicate p) {
    ...
    if (lhs instanceof Variable) {
      ...
      if (c != null && c.isSorted()) return c.getAbsPos();
    }
  ...
  boolean hasCmpNext(Predicate p) {
    return getNextCmpOp(p) != null;
  }

  CmpOp getNextCmpOp(Predicate p) {
    if (p.getPos() == rule.getBody().size() - 1) return null;
    Object next = rule.getBody().get(p.getPos() + 1);
    if (!(next instanceof Expr)) return null;
    Op op = ((Expr) next).root;
    if (!(op instanceof CmpOp)) return null;
    return (CmpOp) op;
  }
  ...
  boolean iteratePart(Predicate p, Set<Variable> resolved) {
    ...
    if (cmpOp.getOp().equals("==")) return false; // should be handled by iterate_by
  ...
  boolean isInPrevColGroup(Predicate p, Variable v, Object param) {
    ...
    for (ColumnGroup cg : colGroups) {
      for (int i = cg.startIdx(); i <= cg.endIdx(); i++) {
        if (p.inputParams()[i].equals(v)) colGroupIdxOfV = cg.startIdx();
        if (p.inputParams()[i].equals(param)) colGroupIdxOfParam = cg.startIdx();
      }
    }
    assert (colGroupIdxOfV >= 0 && colGroupIdxOfParam >= 0);
    return colGroupIdxOfParam < colGroupIdxOfV;
  }
  ...
  boolean requireBroadcast(Predicate p) {
    if (p instanceof PrivPredicate) {
      return false;
    }
    return !isResolved(p, p.first());
  }
  ...
  ST genUpdateCode(ST code) {
    ...
    code.add(
        "stmts",
        headT.className() + " " + _headTable + "=" + headTableVar() + headTablePartition());
  ...
  ST handleErrorFor(AssignOp assign, ST code) {
    ...
    for (int i = 0; i < f.getArgs().size(); i++) {
      ...
      if (MyType.javaType(a).equals(String.class)) a = "\"\\\"\"+" + a + "+\"\\\"\"";
      msg += "+" + a;
    }
    msg += "+\"), \"" + "+_$e";
    msg += "+\"\"";
  ...
  ST insertExprCode(ST code, Expr expr) {
    if (expr.root instanceof AssignOp) {
      ...
      if (assign.multiRows()) code = code2; // returning body of an iterator-loop
    } else {
      ST if_ = tmplGroup.getInstanceOf("if");
  ...
  boolean updateFromRemoteHeadT() {
    if (rule.getBodyP().size() == 1) {
      ...
      return onlyT instanceof RemoteHeadTable;
    }
    return false;
  }
  ...
  boolean insertContinuationCode(ST code, Predicate prevp, Predicate p, Set<Variable> resolved) {
    if (code == null) {
      assert (prevp == null);
      return false;
    }
  ...
  String constraintVar(Predicate p) {
    return "$constr" + p.getPos();
  }
  ...
  String myPartitionIdx() {
    return "$selectedTablePartitionIdx";
  }
