...
public class VisitorCodeGenTest {
  ...
  static Epoch findEpoch(Analysis an, Rule r) {
    for (Epoch e : an.getEpochs()) {
      if (e.getRules().contains(r)) return e;
    }
  ...
  static void ensureExistTableCode(List<Table> table) {
    try {
      TableCodeGen.ensureExist(table);
  ...
  static void createInst() {
    ...
    Joiner[] visitors = c.getRuntime().getJoinerBuilder(r.id()).getNewJoinerInst(r.id());
  ...
  static void iterateRange() {
    String query =
        "Edge(int s:1..1000, (int t)) sortby t.\n"
            + "Clique(int x, int y, int z) sortby x.\n"
            + "Clique(x, y, z) :- Edge(x, y), x<y, Edge(y, z), y<z, Edge(x, z).";
    Analysis an = parseAndAnalysis(query);
    CodeGenMain c = new CodeGenMain(an, SRuntime.create());
    c.generate();
  }

  static void iterateRange2() {
    String query =
        "Edge(int s:1..1000, int a, (int s2:1..100, int t)).\n"
  ...
  static void iterateWithIndex() {
    String query =
        "RDF(Utf8 s, Utf8 p, Utf8 o) indexby s, indexby p.\n"
    ...
    query =
        "Tmp(int a:0..10, Utf8 x1, Utf8 x2).\n"
  ...
  static void iterateWithIndex2() {
    String query =
        "Foo(int a:0..100, (int b:0..10, double c)).\n"
  ...
  static void testIterCol() {
    String query =
        "InEdge(int t:0..100, (int src)) sortby src. \n"
  ...
  static void testContains() {
    ...
    s[0] = "";
    query =
        "Arr2(int a:0..10, (int b:0..10, int c)) indexby a. \n"
            + "Arr2(1, b, b) :- b=1 . \n"
  ...
  static void testChoice() {
    String query =
        "Diff(int n:0..0, int t) groupby(1).\n"
  ...
  static void testChoice2() {
    String query =
        "Path(int i:0..100, (int depth, int to)). \n"
    ...
    query =
        "Path(s, depth, $choice(to)) :- s=0, depth=1, Edge(s, to), !Path(s, _, to); \n"
    ...
    query =
        "Path(s, depth, $choice(to)) :- s=2, depth=8, Edge(s, to), !Path(s, _, to); \n"
            + " 			  :- Path(_, prevDepth, s), depth=prevDepth+1, Edge(s, to), !Path(s, _, to).\n"
            + "PathCnt(s, $inc(1)) :- Path(s, depth, to).\n"
            + "Depth(0, $max(depth)) :- Path(s, depth, to). \n";
    en.run(query);
  ...
  static void testit2() {
    ...
    String q =
        "InEdge(int t:0.."
            ...
            + "Rank(int n:0.."
            + N
            + ", int i:iter, float val) groupby(2).\n"
  ...
  static void testArgmin() {
    String query =
        "Data(int n:0..40000, double x, double y).\n"
    ...
    for (int i = 0; i < 10; i++) {
      query =
          "Cluster(did, "
              + i
              + ", $argmin(idx, diff)) :- Data(did, x, y), Center(idx, a),  (cx,cy)=$unpack(a),"
