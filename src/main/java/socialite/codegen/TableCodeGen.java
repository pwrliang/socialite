...
public class TableCodeGen {
  ...
  String tableName() {
    return TableUtil.getTablePath(table.className());
  }
  ...
  boolean isDelta() {
    return table instanceof DeltaTable;
  }
  ...
  String genNestedTable() {
    ...
    for (int i = 0; i < nestDepth; i++) {
      ...
      if (hasNested) {
        tmpl.add("nestedTable", table.nestedClassName(i + 1));
      }
  ...
  public static LinkedHashMap<String, byte[]> ensureExist(List<Table> tables)
