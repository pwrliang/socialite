...
public class CodeGenMainTest {
  ...
  static void evalSP() {
    ...
    String query =
        "Edge(int s:1..1768198, (int t)).\n"
            + "Edge(s, t) :- (s,t) = $LoadEdge(\"data/lastfm.txt\").\n";
    LocalEngine en = new LocalEngine();
    en.run(query);

    query =
        "SP(int x:1..1768198, int dist).\n"
            + "SP(s, d) :- s=1, d=0.\n"
  ...
  static void evalSP_dir() {
    String query =
        "Edge(int s:1..1768198, (int t, int d)) sortby d.\n"
  ...
  static void evalMultipleQueryParallel() {
    ...
    en.run(query);
    final String query1 =
        "Bar(a,b) :- Foo(a,x), k=x+107, y=$range(x, k), Foo(y, b). \n" + "?-Bar(a,b).\n";
    final String query2 =
        "Baz(a,b) :- Foo(a,x), k=x+37, y=$range(x, k), Foo(y, b). \n" + "?-Baz(a,b).\n";
    final String query3 =
        "Qux(a,b) :- Foo(a,x), k=x+81, y=$range(x, k), Foo(y, b). \n" + "?-Qux(a,b).\n";
  ...
  public static void main(String[] args) {
    ...
    // evalSP_dir();
    for (int i = 0; i < 10; i++) {
      evalMultipleQueryParallel();
      dot();
    }
    testVisitorClassCache();
    dot();
