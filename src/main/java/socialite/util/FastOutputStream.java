...
public class FastOutputStream extends ObjectOutputStream {
  ...
  private String getShortClassDescr(int idx) {
    if (idx < classDescr.length) return classDescr[idx];
    return "#" + idx;
  }
  ...
  void writeObjectArray(Object a) throws IOException {
    ...
    if (TmpTableInst.class.isAssignableFrom(itemType)) tableArray = true;
    ...
    for (int i = 0; i < array.length; i++) {
      if (array[i] == null) {
        ...
        writeBoolean(true);
        assert array[i] instanceof Externalizable || array[i] instanceof String;
        if (tableArray) {
