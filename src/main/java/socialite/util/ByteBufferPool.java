...
public class ByteBufferPool {
  ...
  static final int smallBufferSize = 132 * 1024;
  static final int bufferSize = 1280 * 1024; // see WorkerConnPool:ConnDesc.SOCK_BUFSIZE

  static final long directBufferAlloc = 1024 * 1024 * (256 + 32);

  static ByteBufferPool theInst = null;

  public static int getBufferQueueSize() {
    return (int) getDirectBufferAlloc() / bufferSize();
  }

  public static long getDirectBufferAlloc() {
    return directBufferAlloc;
  }

  public static int smallBufferSize() {
  ...
  public void free(ByteBuffer bb) {
    if (bb.isDirect()) {
      ...
      if (verbose) {
        allocedBuffer.addAndGet(-bb.capacity());
      }
