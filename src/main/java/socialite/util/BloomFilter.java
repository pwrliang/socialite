...
public class BloomFilter implements Externalizable {
  ...
  public boolean isFull() {
    return bitset.cardinality() * 2 + 1 >= getBitSetSize();
  }

  public boolean add(int v) {
    boolean added = false;
    for (int h : hash(v)) {
      if (!bitset.get(h)) added = true;
      bitset.set(h, true);
    }
    numElements++;
    return added;
  ...
  public int getNumExpectedElements() {
    return (int) (this.getBitSetSize() / bitsPerElement);
  }
